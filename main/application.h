...
#define MAIN_EVENT_SLOW_SPEECH_TIMEOUT  (1 << 14)  // DP-039
#define MAIN_EVENT_INPUT_ACTIVITY        (1 << 15)  // DP-039 Fase 2
...
public:
    ...
    // Reproduce de forma segura un OGG/Opus local solicitado por XiaoZhi Care.
    // Copia los bytes antes de encolar el trabajo en el task principal.
    bool RequestCareReminderAudio(const std::string& target_id,
...
private:
    ...
    std::string care_reminder_audio_cache_;  // DP040B_FASE2B_STRONG_AUDIO_HOOK
    ...
    bool pending_listening_start_ = false;  // Waiting for playback to drain before starting listening (auto/CARE slow mode)
    bool slow_speech_heard_voice_ = false;  // DP-039: prevents ending before the person starts speaking
    int64_t slow_speech_hard_deadline_us_ = 0;  // DP-039 Fase 1B
    int clock_ticks_ = 0;
    ...
    // DP-039: local end-of-turn for slow / paused speech.
    void ArmSlowSpeechTimer(uint64_t timeout_us);
