...
// -----------------------------------------------------------------------------
// DP-039 Fase 2 - detector de actividad a partir del PCM real del microfono.
// -----------------------------------------------------------------------------
void AudioService::UpdateInputActivity(const std::vector<int16_t>& data) {
    if (data.empty() || codec_ == nullptr) {
        return;
    }

    const size_t channels = codec_->input_channels() > 0 ? codec_->input_channels() : 1;
    const size_t count = (data.size() + channels - 1) / channels;
    if (count == 0) {
        return;
    }
    ...
    const int32_t mean = static_cast<int32_t>(sum / static_cast<int64_t>(actual));
    uint64_t abs_sum = 0;
    for (size_t i = 0; i < data.size(); i += channels) {
        int32_t delta = static_cast<int32_t>(data[i]) - mean;
        if (delta < 0) delta = -delta;
        abs_sum += static_cast<uint32_t>(delta);
    }
    const uint32_t level = static_cast<uint32_t>(abs_sum / actual);

    if (input_activity_reset_requested_.exchange(false)) {
        input_activity_noise_floor_ = level > 40 ? level : 40;
        input_activity_attack_frames_ = 0;
        input_activity_release_frames_ = 0;
        input_activity_state_ = false;
        input_activity_detected_.store(false);
        return;
    }

    uint32_t& floor = input_activity_noise_floor_;
    if (floor == 0) floor = level > 40 ? level : 40;

    if (level < floor) {
        floor = (floor * 7U + level) / 8U;
    } else if (!input_activity_state_ || level < floor * 3U) {
        floor = (floor * 255U + level) / 256U;
    }

    if (floor < 40U) floor = 40U;
    if (floor > 8000U) floor = 8000U;

    // DP-039 Fase 2B: thresholds recalibrated from real INMP441 logs.
    // Room noise was around 1200-2400 while speech was typically 3300-5800.
    // A higher release threshold lets normal room noise become SILENCE,
    // while the slightly stronger attack threshold avoids brief noise spikes.
    uint32_t threshold_on = (floor * 21U) / 10U + 120U;
    // DP-039 Fase 2C: reject low-energy room noise from reopening speech.
    if (threshold_on < 3000U) threshold_on = 3000U;

    uint32_t threshold_off = (floor * 19U) / 10U + 120U;
    if (threshold_off < 2200U) threshold_off = 2200U;

    if (!input_activity_state_) {
        input_activity_release_frames_ = 0;
        if (level >= threshold_on) {
            if (input_activity_attack_frames_ < 1000) ++input_activity_attack_frames_;
        } else {
            input_activity_attack_frames_ = 0;
        }

        // Require 180 ms of sustained activity before opening/re-opening speech.
        // This filters short room-noise bursts that were repeatedly cancelling
        // the Fast/Medium/Slow silence timer in real hardware tests.
        if (input_activity_attack_frames_ >= 18) {
            input_activity_state_ = true;
            input_activity_detected_.store(true);
            input_activity_attack_frames_ = 0;
            if (callbacks_.on_input_activity) {
                callbacks_.on_input_activity(true, level, floor);
            }
        }
        return;
    }
    ...
    if (level <= threshold_off) {
        if (input_activity_release_frames_ < 1000) ++input_activity_release_frames_;
    } else {
        input_activity_release_frames_ = 0;
    }

    // 120 ms below the release threshold is enough to start the user's
    // configured silence timer. This makes endpointing feel more immediate.
    if (input_activity_release_frames_ >= 12) {
        input_activity_state_ = false;
        input_activity_detected_.store(false);
        input_activity_release_frames_ = 0;
        if (callbacks_.on_input_activity) {
            callbacks_.on_input_activity(false, level, floor);
        }
    }
}
...
void AudioService::EnableVoiceProcessing(bool enable) {
    ...
    if (enable) {
        input_activity_reset_requested_.store(true);
        input_activity_detected_.store(false);
        if (!InitializeAudioEngine()) {
            return;
        }
        ...
        // DP-039 Fase 1B: AfeAudioEngine clears its internal is_speaking_
        // when processing stops, but AudioService also needs to clear the
        // externally visible cached VAD state.
