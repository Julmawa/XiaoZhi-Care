...
struct AudioServiceCallbacks {
    ...
    // DP039_FASE2_CONFIGURABLE: activity derived from raw microphone PCM.
    std::function<void(bool active, uint32_t level, uint32_t noise_floor)> on_input_activity;
...
public:
    ...
    bool IsInputActivityDetected() const { return input_activity_detected_.load(); }
    bool IsIdle();
    bool IsPlaybackIdle();
...
private:
    ...
    // DP-039 Fase 2: raw PCM activity detector. Mutable detector state is owned
    // by AudioInputTask; atomics are used for cross-task state/reset requests.
    std::atomic<bool> input_activity_detected_{false};
    std::atomic<bool> input_activity_reset_requested_{true};
    uint32_t input_activity_noise_floor_ = 0;
    uint16_t input_activity_attack_frames_ = 0;
    uint16_t input_activity_release_frames_ = 0;
    bool input_activity_state_ = false;
    ...
    void AudioInputTask();
    void UpdateInputActivity(const std::vector<int16_t>& data);
